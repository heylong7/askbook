...
以 Ubuntu 22.04 为例，4GB 内存 + 30GB 磁盘起步。
...
# 5. 验证
uv run askbook ingest examples/docs/seed --collection demo
uv run askbook query ask "askbook 是什么？" --collection demo
```

### systemd 持久化

```ini
# /etc/systemd/system/askbook-dashboard.service
[Unit]
Description=askbook Dashboard
After=network.target ollama.service

[Service]
Type=simple
User=ubuntu
WorkingDirectory=/home/ubuntu/askbook
ExecStart=/home/ubuntu/.local/bin/uv run askbook dashboard --port 8501
Restart=always
RestartSec=10
Environment=ASKBOOK_DATA_DIR=/home/ubuntu/.askbook
...
sudo systemctl enable --now askbook-dashboard
sudo ufw allow 8501/tcp
```

> Dashboard 无内置认证，生产环境建议套 Nginx 反向代理 + Basic Auth。
